...
% Create Disturbed Model Parameter
delta = ureal('delta',0);  % Create Delta within range [-1 1]
var_lb   = 0.5;            % percentage of paramter variabilty
lb_unc       = lb * (1 + var_lb*delta);

% Total mass
M_unc = mb*lb_unc + mw*lw;
% Total inertia
I_unc = Ib + mb*lb_unc^2 + mw*lw^2;
...
% Gain of disturbance (friction) input
Bd = [   0                  ;...
         1/I_unc                ;...
       - 1/(I_unc*Iw)*(I_unc + Iw)  ];
